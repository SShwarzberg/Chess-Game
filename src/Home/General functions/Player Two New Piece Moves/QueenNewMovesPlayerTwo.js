const QueenNewMovesPlayerTwo = (individualPiece, boardLetters, playerTwoPiecePositions, playerOnePiecePositions) => {
    ...
    let addToAvailableMovesDiagonal = []
    const checkIfBlockingOwnPieces = () => {
                    ...
                    if (piece.i === Math.min(...downRightIndex)) {
                        directionIndex.forEach(direction => {
                            if (!playerOneTilePositions.some(p1Position => p1Position === direction && p1Position[1] < piece.move[1])) {
                                addToAvailableMovesDiagonal.push(piece.move)
                            } else {
                                blocking = true
                            }
                        })
                        if (blocking === true) {
                            addToAvailableMovesDiagonal = []
                        }
                    }
                })
    ...
    return [returnedMoves, blockingKingFromCheck, tilesBetweenKingAndAttacker, attackingPiecesPositions, attackingPiecesPositionsDiagonal, ownBlockingKingFromCheck]
}
